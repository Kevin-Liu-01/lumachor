...
export function ContextLibraryDock({
  ...
  const {
    ...
    open ? "/api/contexts?withMeta=1&mine=1" : null,
    fetcher
  );
  const {
    data: starredData,
    ...
    open ? "/api/contexts?withMeta=1&starred=1" : null,
    fetcher
  );
  const {
    data: publicData,
    ...
    return mine.map((m) => {
      ...
      return p
        ? {
            ...m,
            owner: true,
            publicId: p.publicId ?? m.publicId ?? null,
            publishedAt: p.publishedAt ?? m.publishedAt ?? null,
          }
        : { ...m, owner: true };
    });
  }, [mine, pub]);
    ...
    return starred.map((s) => {
      ...
      return p
        ? {
            ...s,
            owner: true,
            publicId: p.publicId ?? s.publicId ?? null,
            publishedAt: p.publishedAt ?? s.publishedAt ?? null,
          }
        : { ...s, owner: true };
    });
  }, [starred, pub]);
  ...
  const pubForDisplay = useMemo<ContextRowWithMeta[]>(() => {
    ...
    return pub.map((p) => {
      ...
      return owned
        ? {
            ...owned,
            publicId: p.publicId ?? owned.publicId ?? null,
            publishedAt: p.publishedAt ?? owned.publishedAt ?? null,
          }
        : p;
    });
  }, [pub, mineWithPublic]);
  ...
  const allCombined: ContextRowWithMeta[] = useMemo(() => {
    ...
    const extraStarred = starredWithPublic.filter(
      (s) => !mineIds.has(s.id) && !pubMap.has(s.id)
    );
  ...
  const scopeItems = useMemo<ContextRowWithMeta[]>(() => {
    switch (scope) {
      case "all":
        return allCombined;
      case "mine":
        return mineWithPublic;
      case "starred":
        return starredWithPublic;
      case "public":
        return pubForDisplay;
      default:
        return mineWithPublic;
    }
  ...
  const toggleLike = useCallback(
    async (id: string, _isLiked: boolean) => {
  ...
  const unpublishContext = useCallback(
  ...
  const importPublicContext = useCallback(
    async (publicId: string) => {
  ...
  const handleSelect = useCallback(
    async (c: ContextRowWithMeta) => {
      try {
        if (scope === "public" && !c.owner) {
          if (!c.publicId) {
            toast({
              type: "error",
              description: "Missing publicId for import.",
            });
            return;
          }
        ...
        toast({
          type: "error",
          description: e?.message || "Could not use this public context.",
        });
      }
    },
          ...
          <div className="relative border-b bg-background/70 backdrop-blur">
            ...
            <div className="px-3 pb-3 space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="hidden md:block text-xs opacity-70 px-1">
                  ↑/↓, Enter
                </div>
              </div>
            ...
            {isLoading ? (
              <ul className="space-y-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <li
                    key={i}
                    className="relative rounded-2xl border p-3 bg-background/60 overflow-hidden"
                  >
                    <ShimmerOverlay show />
              ...
              <div className="p-4 text-sm border rounded-xl bg-background/60">
                ...
                <div className="opacity-70">
                  Try a different search, switch scopes, or create a new one.
                </div>
              ...
              <ul className="space-y-2">
                {filtered.map((c, idx) => {
                  ...
                  const rowBusy =
                    likeBusy || delBusy || pubBusy || unpubBusy || impBusy;
                              ...
                              <>
                                {c.publicId ? (
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      Unpublish from the public library
                                    </TooltipContent>
                                  </Tooltip>
                                ) : (
                                  <Tooltip>
