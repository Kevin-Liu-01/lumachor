...
export async function DELETE(
  ...
  try {
    ...
    const [pub] = await db.select().from(PublicContext).where(eq(PublicContext.id, publicId)).limit(1);
    if (!pub) return new ChatSDKError('bad_request:database', 'Public context not found').toResponse();

    // fetch its source context to validate ownership
    const [ctx] = await db.select().from(Context).where(eq(Context.id, pub.contextId)).limit(1);
    if (!ctx) return new ChatSDKError('bad_request:database', 'Source context not found').toResponse();
    ...
    await db.delete(PublicContext).where(and(eq(PublicContext.id, publicId)));
